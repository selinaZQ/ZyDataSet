...
- `03_structured/questions_with_answers.jsonl`：2,400 道题的最终配对文件；每行依次包含题干、A～E 选项、答案、解析及两侧来源页信息。
...
- 最终配对 JSONL 共 2,400 行，每道题与对应答案、解析位于同一行。
- 所有题目均具有 A～E 五个选项。
- 答案原书中有 153 条只给答案、未附解析；最终配对文件在 `analysis` 中明确写入“原书未提供解析。”，并以 `analysis_available: false` 标识，避免误认为是遗漏或生成内容。
